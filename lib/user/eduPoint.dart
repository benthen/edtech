...
class _EduPointPageState extends State<EduPointPage> {
  ...
  void getUserDetail() async {
    ...
    setState(() {
      userDetail = detail;
      eduPoint = userDetail['edu_point'];
    });
  }

  void getBagdeFile() async {
    final file = await user.getAllCourseImage();
    setState(() {
      files = file;
      List<String> badgeMap = userDetail['course_finish'].split(', ');
      if (badgeMap.length > 1) {
        for (int i = 0; i < badgeMap.length; i++) {
          for (int j = 0; j < files.length; j++) {
            if (basename(files[j]['path']).split('.')[0] == badgeMap[i]) {
              badge.add({
